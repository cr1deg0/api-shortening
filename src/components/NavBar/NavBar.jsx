...
const NavBar = () => {
	...
	return (
		<nav
			className='flex flex-row items-center justify-between py-8 px-8
    lg:px-40 lg:py-16 lg:justify-start'
        ...
        lg:relative lg:bg-white lg:top-0 lg:flex-row lg:w-full lg:items-center lg:justify-between lg:mr-0 lg:pr-0'
				style={{
					display:
						isMenuOpen | (viewportWidth > 980) ? 'flex' : 'none',
				}}
			>
				<ul
					className='pt-10 pb-8 border-b border-b-violet-light space-y-7 
        lg:flex lg:justify-center lg:pt-0 lg:pb-0 lg:space-y-0 lg:space-x-6 lg:border-none'
				>
					{menuItemsElement}
				</ul>
				<ul
					className='pt-8 pb-10 space-y-5 flex flex-col items-center
        lg:flex-row lg:content-center lg:pt-0 lg:pb-0 lg:space-y-0 lg:space-x-6'
				>
					<LinkItem href='/' addStyle='lg:self-center'>
						Login
					</LinkItem>
					<li className='list-none'>
						<RoundLinkBtn href='/' addStyle={'w-28 py-2'}>
							Sign Up
						</RoundLinkBtn>
					</li>
				</ul>
